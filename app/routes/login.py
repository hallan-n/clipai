...
from infra.persistence.login import (
    insert_login,
    select_by_email,
    select_login,
    update_login,
...
from infra.security import check_hash, create_access_token, decode_token, hashed
...
def get_login(token: dict = Depends(decode_token)):
    ...
    if login:
        logger.error("Erro ao recuperar login")
        raise HTTPException(403, "Email em uso!")
    logger.info("Login recuperado")
    return login
...
def update_login(login: PutLogin, token: dict = Depends(decode_token)):
    ...
    try:
        ...
        logger.info("Login atualizado")
        return success_login
    except Exception as e:
        logger.error("Erro ao atualizar login")
