...
def _get_video_transcribe(video_path: str):
    transcribe = get("video_transcribe")
    if transcribe:
    ...
    add("video_transcribe", json.dumps(result))
    return result


def _get_highlights(transcribe: dict):
    ...
    cuts = [blocks[i][1] for i, sim in enumerate(similarities) if sim < THRESHOLD]
...
def get_highlights_segments(video_path: str):
    transcribe = _get_video_transcribe(video_path)
    return _get_highlights(transcribe["segments"])
